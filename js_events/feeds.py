...
class LatestEventsFeed(Feed):
    ...
    def __call__(self, request, *args, **kwargs):
        ...
        self.valid_languages = get_valid_languages(
            ...
            site_id=site_id)
        return super(LatestEventsFeed, self).__call__(
            request, *args, **kwargs)

    def link(self):
        return reverse('{0}:event-list-feed'.format(self.namespace))

    def title(self):
        msgformat = {'site_name': Site.objects.get_current().name}
        return _('Events on %(site_name)s') % msgformat

    def get_queryset(self):
        qs = Event.objects.published().namespace(self.namespace).translated(
...
class CategoryFeed(LatestEventsFeed):
